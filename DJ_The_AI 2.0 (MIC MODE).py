...
pygame.display.set_caption("DJ The AI (MIC MODE)")
...
def microphone_listener():
    global last_input_time
    """Continuously listen to the microphone and send recognized speech to handle_command()."""
    global mic
    ...
    try:
        ...
        print("Install PyAudio and make sure a microphone is available.")
        return
    ...
    try:
        with mic as source:
            ...
            print("Microphone ready. Speak commands.")
        ...
        while True:
            ...
            # Do not listen while DJ is speaking. This prevents the microphone
            # from immediately recognizing the DJ's own WAV response.
            if audio_playing.is_set():
                time.sleep(0.1)
                continue
            ...
            try:
                ...
                # Ignore speech that started while the DJ was talking.
                if audio_playing.is_set():
                    continue
                ...
                try:
                    ...
                    # Microphone heard something, but speech was not understood.
                    pass
...
def main():
    ...
    threading.Thread(target=microphone_listener, daemon=True).start()
    ...
    while True:
        ...
        # Commands are received exclusively through the microphone.
        font = pygame.font.SysFont(None, 24)
        mic_text = "MICROPHONE: LISTENING" if mic_ready.is_set() else "MICROPHONE: STARTING..."
        mic_surf = font.render(mic_text, True, (255,255,255))
        screen.blit(mic_surf, (10, HEIGHT - 30))
